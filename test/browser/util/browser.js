...
async function getMapCanvas(url, config) {
    ...
    config = extend({
        ...
        style: {
            ...
            sources: {
                land: {
                    type: 'geojson',
                    data: `${browser.basePath}/test/browser/fixtures/land.json`
                }
            },
            layers: [
                {
    ...
    await browser.driver.executeAsyncScript((config, callback) => {
        /* eslint-disable no-undef */
        const map = window.map = new mapboxgl.Map(config);
